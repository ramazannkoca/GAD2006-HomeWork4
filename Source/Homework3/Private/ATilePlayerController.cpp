...
void AATilePlayerController::BeginPlay()
{
	Super::BeginPlay();
...
void AATilePlayerController::Tick(float DeltaTime)
{
	...
	if (DeprojectMousePositionToWorld(WorldLocation, WorldDirection))
	{
		if (GWorld->LineTraceSingleByChannel(HitResult, WorldLocation, WorldLocation + WorldDirection * 50000.0f, ECC_Visibility))
		{
				...
				GameManager->OnActorInteractıon(HitActor, HitResult.Location, PlayerInput->IsPressed(EKeys::LeftMouseButton));
